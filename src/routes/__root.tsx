...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      {
        title: "Testing 实战学习 v3 · 系统路径",
      },
      {
        name: "description",
        content:
          "前端测试中文交互教程：对齐 Vitest 官方 llms 与 Playwright/RTL 文档：路径、文档地图、Demo 与工坊。",
      },
    ],
    links: [
...
const CTP_BOOT =
  "(function(){try{var f=localStorage.getItem('test-learn-ctp-flavor');var a=localStorage.getItem('test-learn-ctp-accent');var okF=['mocha','macchiato','frappe','latte'];var okA=['green','mauve','blue','lavender','sapphire','teal','peach','pink'];if(okF.indexOf(f)<0)f='mocha';if(okA.indexOf(a)<0)a='peach';document.documentElement.setAttribute('data-ctp-flavor',f);document.documentElement.setAttribute('data-ctp-accent',a);}catch(e){document.documentElement.setAttribute('data-ctp-flavor','mocha');document.documentElement.setAttribute('data-ctp-accent','peach');}})();";
...
function AppShell({ children }: { children: ReactNode }) {
  ...
  const [expanded, setExpanded] = useState<Record<string, boolean>>(() => {
    ...
    for (const tr of orderedTracks()) init[tr] = tr === contTrack || tr === "基础概念";
    return init;
  });
