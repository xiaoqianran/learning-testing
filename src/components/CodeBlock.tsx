...
function colorLine(line: string) {
  ...
  const parts = line.split(
    /(\b(?:import|from|const|let|function|return|export|type|interface|if|else|new|await|async|describe|it|test|expect|vi|render|screen|userEvent|page|browser)\b|'[^']*'|"[^"]*"|`[^`]*`)/g,
  );
  return parts.map((p, i) => {
    if (!p) return null;
    if (
      /^(import|from|const|let|function|return|export|type|interface|if|else|new|await|async)$/.test(
        p,
      )
    ) {
      return (
    ...
    if (/^(describe|it|test|expect|vi|render|screen|userEvent|page|browser)$/.test(p)) {
      return (
        <span key={i} className="text-pass">
          {p}
        </span>
      );
    }
